...
def load_data_federated_by_class(dataset_name,clientID,numClients,basicNN,modelType,trPer,labels):
    ...
    for i in labels:
        ...
        begin_slice_train = int(len(Y_train_current)/(numClients*10/len(labels))*(clientID-1))
        end_slice_train = int(len(Y_train_current)/(numClients*10/len(labels))*clientID)
        begin_slice_test = int(len(Y_test_current)/(numClients*10/len(labels))*(clientID-1))
        end_slice_test = int(len(Y_test_current)/(numClients*10/len(labels))*clientID)

...
for index in range(1,11):
    X,Y,data_dict['x'+str(index)],data_dict['y'+str(index)] = load_data_federated_IID(dataset_name,index,10,0,0,0.8)
...
for index in range(len(data_dict['x'+str(data)])):
    partial_result = []
    for key in ova_model.keys():
        partial_result.append(ova_model[key].predict(tf.reshape(data_dict['x'+str(data)][index],[1,32,32,3])))

...
# Fit the model
model.fit(x_train, y_train, validation_split=0.2, epochs=numEpochs,batch_size=32,steps_per_epoch=int(len(x_train)/160))

# Save the model for later evaluations
model.save('models/centralized_model_'+str(len(x_train))+'_training_samples_'+str(numEpochs)+'_epochs')

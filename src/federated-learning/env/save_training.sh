...
# save results for a robust model without clustering
if [ $ova -eq 0 ];
then
	...
	do
		python3.9 client.py 1 $serverPort $i 0 $numClients $data $scenario $labels $CID >> results/result-$data-complex-model-epochs-$epochs-clients-$numClients-client$i-clusterID-$CID &
	done
fi	

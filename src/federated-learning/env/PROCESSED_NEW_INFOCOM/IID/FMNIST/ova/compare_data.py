...
#        partial_result.append(ova_model[key].predict(tf.reshape(x[index],[1,32,32,1])))
...
for index in range(1,11):
    with open("result-"+str(index),"rb") as read_file:
        y_hat = load(read_file)

    with open("y-"+str(index),"rb") as read_file:
        y = load(read_file)

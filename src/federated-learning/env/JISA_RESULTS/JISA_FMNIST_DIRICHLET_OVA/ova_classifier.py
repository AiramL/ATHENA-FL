...
for index in range(1,11):
    ...
    X,Y,x,y = load_dirichlet_data(dataset_name,data,1,0,0.85)
...
for index in range(len(x)):
    partial_result = []
    for mid in range(10):
        partial_result.append(max(ova_model['model_'+str(mid)].predict(tf.reshape(x[index],[1,32,32,1])).tolist()))
    #result[data-1].append(partial_result.index(max(partial_result)))    
#    print(partial_result, "printing partial result")
#    print(max(partial_result), "printing max partial result")
